...
class CurrencyViewModel {
    ...
    class CurrencyModel: ReactiveCompatible {
        ...
        private let variable: BehaviorRelay<Currency> = .init(value: Currency.null)
        
        var currency: Currency {
            get { return variable.value }
            set { variable.accept(newValue) }//variable.value = newValue }
        }
        ...
        func asObserver() -> AnyObserver<Currency> {
            ...
            //    }.asObserver()
            return Binder(self) { model, currency in
                model.currency = currency
            }.asObserver()
        }
    ...
    class NumberModel: ReactiveCompatible {
        ...
        fileprivate let symbol : BehaviorRelay<String> = .init(value: "")
        fileprivate let number : BehaviorRelay<String> = .init(value: "")
        
        var stringNumber: String {
            get { return number.value }
            set {
                if Double(newValue) != nil { number.accept(newValue) }//{ number.value = newValue }
                else { number.accept("") }//{ number.value = "" }
            }
        }
        ...
        func deleteLastDigit() {
            ...
            number.accept(n >= 10 ? String(Int(n / 10)) : "")
        }
        ...
        func setSlide(slide: Float) {
            ...
            number.accept(intNumber <= 0 ? "" : String(intNumber))
        }
        ...
        func asObserver() -> AnyObserver<Double> {
            ...
            //    }.asObserver()
            return Binder(self) { model, doubleNumber in
                model.doubleNumber = doubleNumber
            }.asObserver()
        }
    ...
    init() {
        ...
        currency.observable.observe(on: MainScheduler.instance)
            .map { $0.symbol }
            .bind(onNext: number.setCurrencySymbol(symbol:))
...
extension Reactive where Base: CurrencyViewModel.NumberModel {
    ...
    var digitCount: Observable<Int> {
        ...
        return base.formattedNumberObservable.map { Int($0 + 0.5) }.map { String($0) }.map { $0.count }
    }
