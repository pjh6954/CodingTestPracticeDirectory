...
class CurrencyFactory: ReactiveCompatible {
    ...
    // Need check this :: https://stackoverflow.com/questions/52793150/rxswift-behaviorrelay-in-place-of-variable-usage
    //fileprivate let rateDate = Variable<Date?>(nil)
    fileprivate let rateDate = BehaviorRelay<Date?>(value: nil)
    // fileprivate var rates: Variable<[String: CurrencyRate]> = Variable([:])
    fileprivate var rates: BehaviorRelay<[String: CurrencyRate]> = BehaviorRelay(value: [:])
...
extension CurrencyFactory {
    ...
    fileprivate func restore() {
        ...
        rateDate.accept(Settings.instance.rateDate)
        rates.accept(ratesValue)
    }
...
extension Reactive where Base : CurrencyFactory {
    var currencies: Observable<[Currency]> {
        return base.rates.asObservable().filter {
            $0.count > 0
        }.map {
            $0.values.map {
                $0.currency
        ...
        return Observable.create { [weak base] observer in
            if let sbase = base {
                ...
                if rates.count > 1 {
                    ...
                    sbase.rates.accept(rates)
                    let formatter = DateFormatter()
                    formatter.dateFormat = "yyyy-MM-dd"
                    //sbase.rateDate.value = formatter.date(from: json["date"].stringValue)
                    sbase.rateDate.accept(formatter.date(from: json["date"].stringValue))
